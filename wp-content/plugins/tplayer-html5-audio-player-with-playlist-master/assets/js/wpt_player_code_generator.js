jQuery(document).ready(function($){
	...
	$('#wpt_player_add_item').click(function(){
		...
		$("#wpt_player_list").append(wpt_player_code);
	});
...
// Используем                    
	sortable( document.getElementById('wpt_player_list'), function (item){
		wpt_player_change_code();
	});
	
	function wpt_player_change_code(){
		wpt_player_list_items=$('#wpt_player_list>li').length;
		var wpt_player_code = [];
		
		for (i=1;i<=wpt_player_list_items;i++){
			wpt_player_id=$('#wpt_player_list>li:nth-child('+i+')').attr('data-id');
